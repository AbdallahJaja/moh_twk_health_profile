...
window.loadVitals = function() {
  ...
  // Fetch vitals (either mock or via API)
  ...
  // Helper: Convert Gregorian date to Hijri (simple approximation; replace with a robust library in production)
  ...
  // Helper: Toggle the "Last Updated" date inline between Gregorian and Hijri
  ...
  function openEditModal(vital) {
    // Save original value for restore default.
    ...
    const modalHtml = `
      <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog" style="position: fixed; bottom: 0; margin: 0; width: 100%; height:400px; align-items:flex-start;">
          <div class="modal-content" style="height:100%;">
            <div class="modal-header d-flex align-items-center">
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="${currentLang === "ar" ? "إغلاق" : "Close"}" style="order:-1;"></button>
            ...
            <div class="modal-footer">
              <button type="button" id="restoreDefaultBtn" class="btn btn-secondary">${currentLang === "ar" ? LANG_AR.restoreDefault : LANG_EN.restoreDefault}</button>
              <button type="button" id="cancelEditBtn" class="btn btn-secondary" data-bs-dismiss="modal">${currentLang === "ar" ? LANG_AR.cancel : LANG_EN.cancel}</button>
              <button type="button" id="saveEditBtn" class="btn btn-primary">${currentLang === "ar" ? LANG_AR.save : LANG_EN.save}</button>
            </div>
          </div>
        </div>
      </div>`;
    ...
    document.getElementById("cancelEditBtn").onclick = () => modalInstance.hide();
    modalEl.addEventListener('hidden.bs.modal', () => modalEl.remove());
    modalInstance.show();
  }

  // Render vitals as clickable cards.
  function renderVitals(vitals) {
    ...
    vitals.forEach((vital, index) => {
      ...
      favBtn.onclick = e => {
        ...
        favImg.src = vital.favorite ? "images/favorite_active.png" : "images/favorite.png";
        // Re-sort vitals to keep favorites on top.
      ...
      if (vital.type === "numeric") {
        ...
        if (vital.list_format === "string") {
          ...
          valueEl.innerHTML = vital.value.map(item => `${item.name} (${item.date}, ${item.dose})`).join("<br>");
        }
      ...
      // --- Card Footer: Last Updated and action icons ---
      const cardFooter = document.createElement("div");
      cardFooter.className = "card-footer d-flex justify-content-between align-items-center";
      
      // Left container: Last updated date with Hijri toggle (on left side)
      ...
      // Right container: Copy and Edit icons.
      ...
      copyBtn.onclick = e => {
        ...
        if (vital.type === "list") {
          if (vital.list_format === "string") {
            ...
            textToCopy = vital.value.map(item => `${item.name} (${item.date}, ${item.dose})`).join("; ");
          }
        } else {
      ...
      // Make the entire card clickable to navigate to vitalDetails.html
  ...
  // Helper: Calculate digital twin insights.
  ...
  // Attach search filtering via Fuse.js.
  ...
  // Main loader: fetch and render vitals.
  function loadVitalsInternal() {
    if (loadingEl) loadingEl.style.display = "block";
    return fetchVitals().then(data => {
      if (!data) {
  ...
  return loadVitalsInternal().then(() => {
        ...
        <div class="modal-content" style="height:100%;">
          ...
          <div class="modal-footer">
            <button type="button" id="restoreDefaultBtn" class="btn btn-secondary">
              ${currentLang === "ar" ? LANG_AR.restoreDefault : LANG_EN.restoreDefault}
            </button>
